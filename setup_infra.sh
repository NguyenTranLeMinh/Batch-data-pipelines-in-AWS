...
echo "AWS_EC2_INSTANCE_NAME="$AWS_EC2_INSTANCE_NAME""
...
echo "Add outbound rule to allow our IP $MY_IP to connect to EC2's $AIRFLOW_UI_PORT port"
aws ec2 authorize-security-group-egress --group-id $EC2_SECURITY_GROUP_ID --protocol tcp --port $AIRFLOW_UI_PORT --cidr $MY_IP/32 --output text >> setup.log
...
aws iam create-role --role-name $REDSHIFT_IAM_ROLE --assume-role-policy-document file://trust-policy.json --description 'spectrum access for redshift' >> setup.log

echo "Attaching AmazonS3ReadOnlyAccess Policy to our IAM role"
aws iam attach-role-policy --role-name $REDSHIFT_IAM_ROLE --policy-arn arn:aws:iam::aws:policy/AmazonS3ReadOnlyAccess --output text >> setup.log
echo "Attaching AWSGlueConsoleFullAccess Policy to our IAM role"
aws iam attach-role-policy --role-name $REDSHIFT_IAM_ROLE --policy-arn arn:aws:iam::aws:policy/AWSGlueConsoleFullAccess --output text >> setup.log

REDSHIFT_SECURITY_GROUP_ID=$(aws ec2 create-security-group --description "Security group to allow inbound RedShift connection" --group-name $REDSHIFT_SECURITY_GROUP --output text)
# Allow 2 inbound source: MY_IP and EC2 which has $EC2_SECURITY_GROUP_ID (Metabase run inside this EC2)
aws ec2 authorize-security-group-ingress --group-id $REDSHIFT_SECURITY_GROUP_ID --protocol tcp --port 5439 --cidr $MY_IP/32 --source-group $EC2_SECURITY_GROUP_ID --output text >> setup.log
echo 'REDSHIFT_SECURITY_GROUP_ID="'$REDSHIFT_SECURITY_GROUP_ID'"' >> state.log

echo "Creating an AWS Redshift Cluster named "$SERVICE_NAME""
aws redshift create-cluster --cluster-identifier $SERVICE_NAME --node-type dc2.large --master-username $REDSHIFT_USER --master-user-password $REDSHIFT_PASSWORD --cluster-type single-node --publicly-accessible --iam-roles "arn:aws:iam::"$AWS_ID":role/"$REDSHIFT_IAM_ROLE"" --vpc-security-group-ids $REDSHIFT_SECURITY_GROUP_ID >> setup.log
...
FROM DATA CATALOG DATABASE 'spectrumdb' iam_role 'arn:aws:iam::"$AWS_ID":role/"$REDSHIFT_IAM_ROLE"' CREATE EXTERNAL DATABASE IF NOT EXISTS;
...
sleep 60
ssh -o "IdentitiesOnly yes" -i "sde-key.pem" ubuntu@$EC2_IPV4 -N -f -L $AIRFLOW_UI_PORT:$EC2_IPV4:$AIRFLOW_UI_PORT
